// Offline cache for Estre's Christening invite.
...
var CACHE_NAME = 'estre-christening-v1';
...
self.addEventListener('fetch', function (event) {
  ...
  if (url.origin === self.location.origin) {
    ...
    // Photos/audio: URLs are unique per upload and never edited in place,
    // so it's safe to serve straight from cache once seen.
    event.respondWith(cacheFirst(req));
  } else if (url.hostname === 'api.jsonbin.io') {
    // RSVP/wishes/gift-registry/gallery data: prefer fresh, fall back to
    // the last-known snapshot when there's no signal.
    event.respondWith(networkFirst(req));
  }
});
